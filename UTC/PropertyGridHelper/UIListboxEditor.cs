	...
	{				
		...
		[RefreshProperties(RefreshProperties.All)]public override object EditValue(System.ComponentModel.ITypeDescriptorContext context, System.IServiceProvider provider, object value)
		{
			...
			if (oEditorService != null)
			{
                ...
                oList.MouseClick += SelectedItem;
                this.oList.Leave += new System.EventHandler(this.SelectedItem); //added to work when enter pressed

				oEditorService.DropDownControl(oList);
				if (oList.SelectedIndices.Count == 1)                
				{
					cp.SelectedItem = oList.SelectedItem;
                    DataRowView r = oList.SelectedItem as DataRowView;
                    //MessageBox.Show(r.Row[oList.ValueMember].ToString());
                    if (r != null && oList.ValueMember.Length >0)
                        cp.SelectedValue = r.Row[oList.ValueMember]; // oSelectedValue;
					value = oList.Text;
				}
